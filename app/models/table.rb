class Table < ActiveRecord::Base
  ...
  def clear
    ...
    self.defender_cursor = NUMBER_ONE
    self.attacker_cursor = NUMBER_ZERO
  end

  def get_latest_card
    self.table_cards[self.cards_count- NUMBER_ONE]
  end
  ...
  def table_empty?
    if self.cards_count > NUMBER_ZERO
      FALSE
    else
      TRUE
    end
